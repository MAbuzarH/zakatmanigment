...
$sqliAmout = 'SELECT * FROM payment';
$resultAmount = mysqli_query($conn, $sqliAmout) or die('Error');

$currentPage = isset($_GET['page']) ? $_GET['page'] : 1;
$limi = 10;
$offset = ($currentPage - 1) * $limi;
$query = "SELECT recevier.*, payment.amout,payment.payment_date FROM recevier JOIN payment ON recevier.payment_id = payment.payment_id  LIMIT   {$offset},  {$limi}";
...
<body>
    <div class="d-flex justify-content-center align-items-center h-75 flex-column ">
        <h1 class="text-center pt-4 pb-2">Receivers</h1>
        <div class="cardsp card bg-light col-10 p-4 h-75">
                ...
                <tbody>
                    <tr>
                        ...
                        if (!empty(isset($_POST['search']))) {
                            ...
                            $whereClause = "payment_id LIKE '%$searchTerm%' OR recevier_name LIKE '%$searchTerm%' OR recevier_email LIKE '%$searchTerm%'";
                            $sql = "SELECT recevier.*, payment.amout,payment.payment_date FROM recevier JOIN payment ON recevier.payment_id = payment.payment_id WHERE $whereClause  LIMIT   {$offset},  {$limi}";
                            ...
                            if (
                                ...
                                while ($row = mysqli_fetch_assoc($searchresult)) { ?>
                                    ...
                                    <td>
                                        <?php echo $row['recevier_name'];
                                        ?>
                                    </td>
                                    <td>
                                        <?php echo $row['recevier_email'];
                                        ?>
                                    </td>
                                    <td>
                                        <?php echo $row['recevier_phn'];
                                        ?>
                                    </td>
                                    <td>
                                        <?php echo $row['recevier_addres'];
                                        ?>
                                    </td>
                                    <td>
            ...
            <tr>
                ...
                ?>
                    ...
                    <!-- <td>
            <a href="view/index.php?id=<?php echo $row['recevier_id']; ?>" class="btn btn-info" type="view" name="view" id="view"><i class="fa-solid fa-eye"></i></a>
          </td>
          <td>
